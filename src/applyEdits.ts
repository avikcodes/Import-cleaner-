...
import { UnusedImport } from './unusedImportAnalyzer';
 ...
 */
export async function applyUnusedImportEdits(editsByFile: Map<string, UnusedImport[]>): Promise<void> {
    ...
    logger.info(`Detected ${totalImportsCount} unused items across ${editsByFile.size} file(s)`);

    const selection = await vscode.window.showInformationMessage(
        `Tidy Imports: Found ${totalImportsCount} unused item(s) across ${editsByFile.size} file(s). Clean them now?`,
        'Clean',
        'Cancel'
    );

    if (selection !== 'Clean') {
        logger.info('User cancelled removal operation');
        return;
    }

    logger.info('User confirmed cleanup. Normalizing and applying edits...');
    ...
    for (const [fileUriStr, unusedItems] of editsByFile) {
        ...
        try {
            ...
            logger.error(`Could not open document ${fileUriStr}`, err);
            continue;
        }
        ...
        for (const declaration of importDeclarations) {
            ...
            if (unusedForThisDecl.length === 0) continue;
            ...
            let namedImports: { name: string, propertyName?: string }[] = [];
            ...
            if (!defaultName && !namespaceBinding && namedImports.length === 0) {
                ...
                if (namespaceBinding) {
                    ...
                    if (namedImports.length > 0) {
                        newImportStr += '{ ' + namedImports.map(ni =>
                            ni.propertyName ? `${ni.propertyName} as ${ni.name}` : ni.name
                        ).join(', ') + ' }';
                    }
                }
                ...
                const range = new vscode.Range(
                    document.positionAt(declaration.getStart()),
                    document.positionAt(declaration.getEnd())
                );
    ...
    if (success) {
        logger.info(`Successfully processed and normalized unused imports`);
        vscode.window.showInformationMessage(`Successfully removed unused import(s).`);
    } else {
        logger.error('Failed to apply normalized workspace edits');
 ...
 */
function getFullDeclarationRange(document: vscode.TextDocument, node: ts.Node): vscode.Range {
